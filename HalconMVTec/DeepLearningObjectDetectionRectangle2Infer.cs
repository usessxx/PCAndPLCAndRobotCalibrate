...
{
    public class DeepLearningObjectDetectionRectangle2Infer
    {
        ...
        HTuple hv_bbox_row = new HTuple();
        HTuple hv_bbox_col = new HTuple();
        HTuple hv_bbox_length1 = new HTuple();
        HTuple hv_bbox_length2 = new HTuple();
        ...
        #region DeepLearningObjectDetectionRectangle2Infer(构造函数里对推理参数初始化)
        //构造函数里对评估参数初始化
        public DeepLearningObjectDetectionRectangle2Infer()
        {
            hv_BatchSize = 1;
            hv_UseGPU = 1;
        }
        ...
        public string InferAction(string preprocessResultDirectoryPath, string preprocessResultParameterFileName, string lableDataSetFilePath,string bestModelBaseName, int batchSize, bool useGPU, HObject ho_Image_Infer, bool needReadparameter, bool needDispose)
        {
            try
            {
                ...
                hv_bbox_row.Dispose();
                HOperatorSet.GetDictTuple(hv_DLResultBatch, "bbox_row", out hv_bbox_row);
                hv_bbox_col.Dispose();
                HOperatorSet.GetDictTuple(hv_DLResultBatch, "bbox_col", out hv_bbox_col);
                hv_bbox_length1.Dispose();
                HOperatorSet.GetDictTuple(hv_DLResultBatch, "bbox_length1", out hv_bbox_length1);
                hv_bbox_length2.Dispose();
                HOperatorSet.GetDictTuple(hv_DLResultBatch, "bbox_length2", out hv_bbox_length2);
                hv_bbox_phi.Dispose();
                HOperatorSet.GetDictTuple(hv_DLResultBatch, "bbox_phi", out hv_bbox_phi);
                ...
                if ((int)(new HTuple(hv_bbox_row.TupleLength())) > 0)
                {
                    //计算model和原图的缩放比例
                    ...
                    {
                        ho_Rectangle.Dispose();
                        HOperatorSet.GenRectangle2ContourXld(out ho_Rectangle, hv_H_R * hv_bbox_row,
                            hv_W_C * hv_bbox_col, hv_bbox_phi, hv_W_C * hv_bbox_length1, hv_H_R * hv_bbox_length2);
                    }

                    //xld
                    ho_RegionBorder.Dispose();
                    //与水平矩形不同处
                    //HOperatorSet.Boundary(ho_Rectangle, out ho_RegionBorder, "inner");----为什么出错???
                    ...
                    for (int i = 0; i < hv_bbox_row.TupleLength(); i++)
                    {
                        int kk = (int)hv_bbox_class_id[i];
                        hv_Progress_ForDisplay[i + 1] = "class_id:" + kk.ToString() + ", class_name:" + hv_ClassNames[kk] + ", confidence:" + hv_bbox_confidence[i] + ", phi:" + hv_bbox_phi[i];
                        string dispalyColor = "red";
                        if (kk == 0)
                            dispalyColor = "green";

                        HOperatorSet.SetDraw(hv_HWindowControl.HalconWindow, "margin");
                        HOperatorSet.SetColor(hv_HWindowControl.HalconWindow, dispalyColor);
                        //与水平矩形不同处
                        HOperatorSet.DispObj(ho_Rectangle, hv_HWindowControl.HalconWindow);
                        set_display_font(hv_HWindowControl.HalconWindow, 20, "mono", "false", "false");
                        HOperatorSet.DispText(hv_HWindowControl.HalconWindow, kk.ToString(), "image", hv_H_R * hv_bbox_row[i], hv_W_C * hv_bbox_col[i], "yellow", "box", "false");
                    }          
                }
                else
                {
                ...
                set_display_font(hv_HWindowControl.HalconWindow, 8, "mono", "false", "false");
                HOperatorSet.DispText(hv_HWindowControl.HalconWindow, hv_Progress_ForDisplay, "image", 10, 10, "black", "box", "false");
