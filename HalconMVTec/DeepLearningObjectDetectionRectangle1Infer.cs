...
{
    public class DeepLearningObjectDetectionRectangle1Infer
    {
        ...
        HTuple hv_Model_ImageWidth= new HTuple();
        HTuple hv_Model_ImageHeight= new HTuple();
        ...
        HObject ho_RegionBorder = null;
        HTuple hv_DictHandle = new HTuple();

        HTuple hv_MinConfidence = new HTuple();
        HTuple hv_MaxOverlap = new HTuple();
        HTuple hv_MaxOverlapClassAgnostic = new HTuple();

        HTuple hv_bbox_row1 = new HTuple();
        HTuple hv_bbox_col1 = new HTuple();
        HTuple hv_bbox_row2 = new HTuple();
        HTuple hv_bbox_col2 = new HTuple();
        HTuple hv_bbox_class_id = new HTuple();
        HTuple hv_bbox_confidence = new HTuple();
        HTuple hv_W_C = new HTuple();
        HTuple hv_H_R = new HTuple();


        //ADD FOR DISPLAY
        public HWindowControl hv_HWindowControl = new HWindowControl();//
        HTuple hv_Progress_ForDisplay = new HTuple();

        public bool displayNextFlag = false;

        #region DeepLearningObjectDetectionRectangle1Infer(构造函数里对推理参数初始化)
        //构造函数里对评估参数初始化
        public DeepLearningObjectDetectionRectangle1Infer()
        {
            hv_BatchSize = 1;
            hv_UseGPU = 1;
        }
       ...
       public struct Result
        {
            public HTuple class_id;
            public HTuple classNames;
            public HTuple confidence;
            public HObject ho_Rectangle;
        }
       public Result inferResult;

        public Result InferAction(string preprocessResultDirectoryPath, string preprocessResultParameterFileName, string lableDataSetFilePath,string bestModelBaseName, int batchSize, bool useGPU, HObject ho_Image_Infer, bool needReadparameter, bool needDispose)
        {
            inferResult.class_id = null;
            inferResult.classNames = null;
            inferResult.confidence = null;
            inferResult.ho_Rectangle = null;
            ...
            try
            {
                if (needReadparameter)//读取参数耗费时间
                {
                    ...
                    HOperatorSet.SetDlModelParam(hv_DLModelHandle, "max_overlap_class_agnostic", hv_MaxOverlapClassAgnostic);
                    ...
                    HOperatorSet.GetDictTuple(hv_DLPreprocessParam, "image_width", out hv_Model_ImageWidth);
                    HOperatorSet.GetDictTuple(hv_DLPreprocessParam, "image_height", out hv_Model_ImageHeight);
                }

                //获得原图的尺寸
                HOperatorSet.GetImageSize(ho_Image_Infer, out hv_Width, out hv_Height);
                ...
                //解析的结果 （基于model尺寸）
                HOperatorSet.GetDictTuple(hv_DLResultBatch, "bbox_row1", out hv_bbox_row1);
                HOperatorSet.GetDictTuple(hv_DLResultBatch, "bbox_col1", out hv_bbox_col1);
                HOperatorSet.GetDictTuple(hv_DLResultBatch, "bbox_row2", out hv_bbox_row2);
                HOperatorSet.GetDictTuple(hv_DLResultBatch, "bbox_col2", out hv_bbox_col2);
                HOperatorSet.GetDictTuple(hv_DLResultBatch, "bbox_class_id", out hv_bbox_class_id);
                HOperatorSet.GetDictTuple(hv_DLResultBatch, "bbox_confidence", out hv_bbox_confidence);
                ...
                if ((int)(new HTuple(hv_bbox_row1.TupleLength())) > 0)
                {
                    //计算model和原图的缩放比例
                    hv_W_C = hv_Width / (float)hv_Model_ImageWidth;
                    hv_H_R = hv_Height / (float)hv_Model_ImageHeight;

                    //生成框框(基于原图）
                    ho_Rectangle.Dispose();
                    HOperatorSet.GenRectangle1(out ho_Rectangle, hv_H_R * hv_bbox_row1, hv_W_C * hv_bbox_col1, hv_H_R * hv_bbox_row2, hv_W_C * hv_bbox_col2);

                    //xld
                    ho_RegionBorder.Dispose();
                    HOperatorSet.Boundary(ho_Rectangle, out ho_RegionBorder, "inner");

                    set_display_font(hv_HWindowControl.HalconWindow, 8, "mono", "false", "false");

                    if (((int)((new HTuple(hv_Progress_ForDisplay.TupleLength()))) == 0) || hv_Progress_ForDisplay == "")
                        hv_Progress_ForDisplay[0] = "NoName";
                    
                    for (int i = 0; i < hv_bbox_row1.TupleLength(); i++)
                    {
                        int kk = (int)hv_bbox_class_id[i];
                        hv_Progress_ForDisplay[i + 1] = "class_id:" + kk.ToString() + ", class_name:" + hv_ClassNames[kk] + ", confidence:" + hv_bbox_confidence[i];
                        if (kk == 2)
                            displayColor = "green";

                        HOperatorSet.SetDraw(hv_HWindowControl.HalconWindow, "margin");
                        HOperatorSet.SetColor(hv_HWindowControl.HalconWindow, displayColor);
                        HOperatorSet.DispObj(ho_RegionBorder, hv_HWindowControl.HalconWindow);
                        set_display_font(hv_HWindowControl.HalconWindow, 20, "mono", "false", "false");
                        //HOperatorSet.DispText(hv_HWindowControl.HalconWindow, kk.ToString(), "image", hv_H_R * hv_bbox_row1[i], hv_W_C * hv_bbox_col1[i], displayColor, "box", "false");
                    }
                    inferResult.class_id = hv_bbox_class_id;
                    inferResult.classNames = hv_ClassNames;
                    inferResult.confidence = hv_bbox_confidence;
                    inferResult.ho_Rectangle = ho_Rectangle;
                }
                else
                {
                    set_display_font(hv_HWindowControl.HalconWindow, 32, "mono", "false", "false");
                    hv_Progress_ForDisplay[1] = "没找到目标";

                    inferResult.class_id = "NA";
                    inferResult.classNames = "NA";
                    inferResult.confidence = 0;
                    inferResult.ho_Rectangle = null;

                    set_display_font(hv_HWindowControl.HalconWindow, 8, "mono", "false", "false");
                    HOperatorSet.DispText(hv_HWindowControl.HalconWindow, hv_Progress_ForDisplay, "image", 10, 10, displayColor, "box", "false");
                    hv_Progress_ForDisplay = "";
                }
            }
            ...
            catch (HalconException HDevExpDefaultException)
            {
                ...
                hv_Model_ImageWidth.Dispose();
                hv_Model_ImageHeight.Dispose();
                hv_MinConfidence.Dispose();
                hv_MaxOverlap.Dispose();
                hv_MaxOverlapClassAgnostic.Dispose();
                hv_UseGPU.Dispose();
                ...
                hv_bbox_row1.Dispose();
                hv_bbox_col1.Dispose();
                hv_bbox_row2.Dispose();
                hv_bbox_col2.Dispose();
                hv_bbox_class_id.Dispose();
                hv_bbox_confidence.Dispose();
                hv_W_C.Dispose();
                hv_H_R.Dispose();
        ...
        public void TestImagesInfer(string preprocessClassificationResultDirectoryPath, string preprocessClassificationResultParameterFileName, string lableDataSetFilePath,string bestModelBaseName, int batchSize, bool useGPU, string testimagesDirectoryPath)
        {
            ...
            for (hv_Index = 0; (int)hv_Index <= (int)((new HTuple(hv_ImageFiles.TupleLength())) - 1); hv_Index = (int)hv_Index + 1)
            {
                ...
                string name = hv_ImageFiles.TupleSelect(hv_Index).S;
                ...
                hv_Progress_ForDisplay = "";//显示信息清零
                hv_Progress_ForDisplay[0] = fileNameWithoutExtension + extension;
                if (hv_Index > 0)
                    needReadparameter = false;
                if ((int)hv_Index == (int)((new HTuple(hv_ImageFiles.TupleLength())) - 1))
                    needDispose = true;

                InferAction(preprocessClassificationResultDirectoryPath, preprocessClassificationResultParameterFileName, lableDataSetFilePath,bestModelBaseName, batchSize, useGPU, ho_Image_Infer, needReadparameter, needDispose);

                string displayColor = "red";
                int kk = -1;
                try
                {
                    kk = inferResult.class_id[0].I;
                }
                catch
                {
                }
                if (kk == 2 && kk != -1)
                    displayColor = "green";
                if (kk != -1)
                    HOperatorSet.DispText(hv_HWindowControl.HalconWindow, fileNameWithoutExtension + "\n" + kk + ": " + inferResult.classNames[kk] + "," + inferResult.confidence, "window", "top", "left", displayColor, "box", "false");
        ...
        // Procedures 
        // External procedures 
        public void add_colormap_to_image(HObject ho_GrayValueImage, HObject ho_Image, out HObject ho_ColoredImage, HTuple hv_HeatmapColorScheme)      
        {
        ...
        public void get_dl_class_colors(HTuple hv_ClassNames, out HTuple hv_Colors)
        {
            ...
            try
            {
                ...
                hv_Colors.Dispose();
                make_neighboring_colors_distinguishable_dl_visualization(hv_ColorsRainbow,
                    out hv_Colors);
        ...
        public void make_neighboring_colors_distinguishable_dl_visualization(HTuple hv_ColorsRainbow,
            out HTuple hv_Colors)
        {
        ...
        public void dev_display_screen_inference_step_1(HTuple hv_ExampleInternals)
        {
            ...
            try
            {
                ...
                if (HDevWindowStack.IsOpen())
                {
                    HOperatorSet.DispObj(ho_ImageRaw, HDevWindowStack.GetActive());
                }
                if (HDevWindowStack.IsOpen())
                {
        ...
        public void dev_display_screen_inference_step_2_part_1(HTuple hv_ExampleInternals)
        {
            ...
            try
            {
                ...
                if (HDevWindowStack.IsOpen())
                {
                    HOperatorSet.SetColor(HDevWindowStack.GetActive(), "light blue");
                }
                if (HDevWindowStack.IsOpen())
                {
                    HOperatorSet.SetLineWidth(HDevWindowStack.GetActive(), 2);
                ...
                if (HDevWindowStack.IsOpen())
                {
                    HOperatorSet.DispObj(ho_ImageTrans, HDevWindowStack.GetActive());
                }
                if (HDevWindowStack.IsOpen())
                {
                    HOperatorSet.DispObj(ho_Loupe1, HDevWindowStack.GetActive());
                }
                if (HDevWindowStack.IsOpen())
                {
                    HOperatorSet.DispObj(ho_Loupe2, HDevWindowStack.GetActive());
                }
                if (HDevWindowStack.IsOpen())
                {
                    HOperatorSet.DispObj(ho_LoupeLine1, HDevWindowStack.GetActive());
                }
                if (HDevWindowStack.IsOpen())
                {
                    HOperatorSet.DispObj(ho_LoupeLine2, HDevWindowStack.GetActive());
                }
                if (HDevWindowStack.IsOpen())
                {
                    HOperatorSet.DispText(HDevWindowStack.GetActive(), "Raw image", "window",
                ...
                //
                hv__.Dispose(); hv__.Dispose(); hv__.Dispose(); hv_TextHeight.Dispose();
                HOperatorSet.GetStringExtents(hv_WindowHandleImages, "test_string", out hv__,
                    out hv__, out hv__, out hv_TextHeight);
                if (HDevWindowStack.IsOpen())
                {
                    using (HDevDisposeHelper dh = new HDevDisposeHelper())
                    {
                        HOperatorSet.DispText(HDevWindowStack.GetActive(), "Original resolution",
                            "image", ((hv_Height - (hv_LoupeZoom * hv_LoupeWindowSize)) - hv_LoupeMargin) - (3 * hv_TextHeight),
        ...
        public void dev_display_screen_inference_step_2_part_2(HTuple hv_ExampleInternals)
        {
            ...
            try
            {
                ...
                if (HDevWindowStack.IsOpen())
                {
                    HOperatorSet.DispObj(ho_ImageZoomed, HDevWindowStack.GetActive());
                }
                HOperatorSet.SetWindowParam(hv_WindowHandleImages, "flush", "false");
                if (HDevWindowStack.IsOpen())
                {
                ...
                if (HDevWindowStack.IsOpen())
                {
                    HOperatorSet.DispText(HDevWindowStack.GetActive(), "Preprocessed image for image size 512 x 320",
                        "window", "top", "left", "black", new HTuple(), new HTuple());
                }
                //
                hv__.Dispose(); hv__.Dispose(); hv__.Dispose(); hv_TextHeight.Dispose();
                ...
                if (HDevWindowStack.IsOpen())
                {
                    using (HDevDisposeHelper dh = new HDevDisposeHelper())
                    {
                        HOperatorSet.DispText(HDevWindowStack.GetActive(), "Reduced resolution",
                            "image", ((hv_Height - (hv_LoupeZoom * hv_LoupeWindowSize)) - hv_LoupeMargin) - ((3 * hv_TextHeight) * hv_ZoomFactor),
                            hv_LoupeMargin, "black", new HTuple(), new HTuple());
                    }
                }
                //
            ...
            HTuple hv_Text)
        {
            ...
            try
            {
                ...
                if (HDevWindowStack.IsOpen())
                {
                    HOperatorSet.DispObj(ho_Box, HDevWindowStack.GetActive());
                }
                if (HDevWindowStack.IsOpen())
                {
                    using (HDevDisposeHelper dh = new HDevDisposeHelper())
                ...
                if (HDevWindowStack.IsOpen())
                {
                    HOperatorSet.SetColor(HDevWindowStack.GetActive(), hv_Colors);
                }
                if (HDevWindowStack.IsOpen())
                {
                    HOperatorSet.DispObj(ho_Box, HDevWindowStack.GetActive());
                ...
                if (HDevWindowStack.IsOpen())
                {
                    HOperatorSet.DispText(HDevWindowStack.GetActive(), hv_Text, "image", hv_Row1,
                        hv_Column1, "white", ((new HTuple("box_color")).TupleConcat("shadow")).TupleConcat(
