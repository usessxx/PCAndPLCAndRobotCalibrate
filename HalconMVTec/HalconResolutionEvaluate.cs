...
{
    public class HalconResolutionEvaluate
    {
        /// <summary>
        /// resolutionEvaluate:清晰度评估
        /// </summary>
        /// <param name="ho_Image">HObject:需要进行清晰度评估的图片</param>
        /// <param name="hv_Method">HObject:清晰度评估方法:Deviation 方差法(效果最差);laplace 拉普拉斯能量函数;energy 能量梯度函数;Brenner 梯度函数(效果最佳);Tenegrad 梯度函数</param>
        /// <param name="hv_resolutionEvaluateValue">HObject:清晰度评估结果,值越大越清晰</param>
        public void resolutionEvaluate(HObject ho_Image, HTuple hv_Method, out HTuple hv_resolutionEvaluateValue)
        {
            //不同的算子不同的效果
            //1、Deviation 方差法(效果最差)  
            //2、laplace 拉普拉斯能量函数    
            //3、energy 能量梯度函数
            //4、Brenner 梯度函数(效果最佳)   
            //5、Tenegrad 梯度函数
            ...
            hv_resolutionEvaluateValue = new HTuple();
            try
            {
                {
                ...
                hv_resolutionEvaluateValue.Dispose();
                hv_resolutionEvaluateValue = new HTuple(hv_Value);
                ho_Image_COPY_INP_TMP.Dispose();
                ho_ImageMean.Dispose();
                ho_ImageSub.Dispose();
                ho_ImageResult.Dispose();
